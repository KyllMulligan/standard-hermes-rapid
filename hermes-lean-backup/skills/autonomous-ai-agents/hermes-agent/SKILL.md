...
5. **GitHub token present but tools still unauthenticated**: put the token in `~/.hermes/.env` under the exact env var your workflow expects (`GITHUB_TOKEN` and/or `GH_TOKEN`), then reload env (`/reload`) or start a new session. Quick verification:
   ```bash
